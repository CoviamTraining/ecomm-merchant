...
public interface MerchantRepository extends CrudRepository<Merchant, Integer> {
    ...
    @Query("select new com.coviam.ecommmerchant.dto.MerchantInfoNameLogoRating( m.name,m.logo,m.rating ) " +
            "from Merchant m where m.id = :id ")
    MerchantInfoNameLogoRating getMerchantNameLogoRating(@Param("id") int id);


    @Query("select new com.coviam.ecommmerchant.dto.MerchantInfoSoldDistinctRating( m.productsold,m.distinctproduct, m.rating ) " +
            "from Merchant m where m.id = :id ")
    MerchantInfoSoldDistinctRating getProductSoldDistinctProductRating(@Param("id") int id);

    @Query("select new com.coviam.ecommmerchant.dto.MerchantInfoSoldDistinct( m.productsold,m.distinctproduct ) " +
            "from Merchant m where m.id = :id ")
